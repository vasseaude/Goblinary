...
namespace Goblinary.TreeGenerator
{
    public class TreeBuilder : IDisposable
    {
                                        ...
                                        ImageFormat ImageType)
        {
            ...
            if(dtTree.Select(string.Format("nodeID='{0}'",StartFromNodeID.Replace("'","''"))).Length>0)
            {
                rootDescription = ((TreeData.TreeDataTableRow) dtTree.Select(string.Format("nodeID='{0}'",StartFromNodeID.Replace("'","''")))[0]).nodeDescription;
                rootNote = ((TreeData.TreeDataTableRow) dtTree.Select(string.Format("nodeID='{0}'",StartFromNodeID.Replace("'","''")))[0]).nodeNote;
            }
        ...
        private void BuildTree(XmlNode oNode, int y)
        {
            ...
            foreach (TreeData.TreeDataTableRow  childRow in dtTree.Select(
                string.Format("parentNodeID='{0}'", oNode.Attributes["nodeID"].InnerText.Replace("'", "''"))))
            {
                if (oNode.ParentNode.ParentNode != null) // if this isn't the top
                {
                ...
                else // this is the top, so do things as originally designed.
                {
                //for each child node call this function again
                childNode = GetXMLNode(childRow.nodeID, childRow.nodeDescription, childRow.nodeNote);
                oNode.AppendChild(childNode);
            ...
            if (oNode.ParentNode.ParentNode != null) // if this isn't the top
            {
                ...
                if (oNode.ParentNode.ParentNode.Name != "#document") // if parent is NOT the top, we want to do things differently
                {
                    if (oNode.PreviousSibling == null) // top of the children
                    {
                        int[] ResultsArr = new int[] {-1, GetXPosByParentPreviousSibling(oNode),
                                    GetXPosByPreviousSibling(oNode),
                                    _Margin };
                        Array.Sort(ResultsArr);
                        StartX = ResultsArr[3] + ((_BoxWidth + _HorizontalSpace)/2);
                        StartY = (y * (_BoxHeight + _VerticalSpace)) + _Margin;
                    }
                    else // not the top of the children (has a previoussibling)
                    {
                        int[] ResultsArr = new int[] {-1, GetXPosByParentPreviousSibling(oNode),
                                    -1,
                                    _Margin };
                        ...
                        StartX = ResultsArr[3] + ((_BoxWidth + _HorizontalSpace)/2);
                        //StartY = (y * (_BoxHeight + _VerticalSpace)) + _Margin;
                        StartY = ((y+(oNode.ParentNode.ChildNodes.Count-1)) * (_BoxHeight + _VerticalSpace)) + _Margin;
                    }
                }
                else // parent IS the top, so let's do things normally
                {
                    int[] ResultsArr = new int[] {-1, GetXPosByParentPreviousSibling(oNode),
                                    GetXPosByPreviousSibling(oNode),
                                    _Margin };
            ...
            else  // so... I guess this is the top?
            {
        ...
        */
        ...
        private void DrawChart(XmlNode oNode)
        {
            ...
            Font drawFont = new Font("verdana", _FontSize, FontStyle.Bold);
            Font drawFont2 = new Font("verdana", _FontSize); // drawFont2 is used exclusively for drawString2 below
            ...
            String drawString2 = oNode.Attributes["nodeNote"].InnerText; // note from Kitsune: this was split so that we can add different formatting to each part
            
            // Draw string to screen.
            gr.DrawString(drawString, drawFont, drawBrush, currentRectangle, drawFormat);
            // Find out how tall that string was
            SizeF stringSize = gr.MeasureString(drawString, drawFont, currentRectangle.Width);
            // Draw a new rectangle that is offset by the height of that string
            RectangleF offsetRectangle = new RectangleF(currentRectangle.Left, currentRectangle.Top + stringSize.Height, currentRectangle.Width, currentRectangle.Height - stringSize.Height);
            // Draw the 2nd string, which has it's own font/format
            gr.DrawString(drawString2, drawFont2, drawBrush, offsetRectangle, drawFormat);
            ...
            if (oNode.ParentNode.ParentNode != null)
            {
                if (oNode.ParentNode.ParentNode.Name != "#document") // if parent is not the top
                {
                    // draw a horizontal line coming out of the left side of the box
                    gr.DrawLine(boxPen, currentRectangle.Left,
                                        currentRectangle.Top + (_BoxHeight / 2),
                                        currentRectangle.Left - (_HorizontalSpace / 2),
                                        currentRectangle.Top + (_BoxHeight / 2));

                    //if (oNode.PreviousSibling == null) // top of the children
                    //{
                    //    // draw a line from the left-horizontal-line up to the gap,
                    //    gr.DrawLine(boxPen, currentRectangle.Left - (_HorizontalSpace / 3),
                    //                        currentRectangle.Top + (_BoxHeight / 2),
                    //                        currentRectangle.Left - (_HorizontalSpace / 3),
                    //                        currentRectangle.Top - (_VerticalSpace / 2));
                    //    // then, draw a line over to the middle
                    //    gr.DrawLine(boxPen, currentRectangle.Left - (_HorizontalSpace / 3),
                    //                        currentRectangle.Top - (_VerticalSpace / 2),
                    //                        getRectangleFromNode(oNode.ParentNode).Left + (_BoxWidth / 2) - (_LineWidth / 2),
                    //                        getRectangleFromNode(oNode.ParentNode).Top + (_BoxHeight) + (_VerticalSpace / 2));

                    //}
                    //else // one of the other children
                    //{
                        // draw line coming from the left-horizontal-line up to the one above it
                        gr.DrawLine(boxPen, currentRectangle.Left - (_HorizontalSpace / 2),
                                            currentRectangle.Top + (_BoxHeight / 2),
                                            currentRectangle.Left - (_HorizontalSpace / 2),
                                            currentRectangle.Top - (_BoxHeight / 2) - _VerticalSpace);
                    //}
                }
                else // parent IS the top box
                {
                    // draw the lines coming out of the tops
                    gr.DrawLine(boxPen, currentRectangle.Left + (_BoxWidth / 2),
                                        currentRectangle.Top,
                                        currentRectangle.Left + (_BoxWidth / 2),
                                        currentRectangle.Top - (_VerticalSpace / 2));
                    if (oNode.PreviousSibling != null) // the parent IS the top box, but not the far left one...
                    {
