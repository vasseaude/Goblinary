<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="Recipe.ascx.cs" Inherits="Goblinary.Website.Controls.Recipe.Recipe" %>
<asp:GridView ID="RecipesGridView" runat="server" CssClass="tablesorter custom-popup" AutoGenerateColumns="false" >
    <Columns>
        ...
        <asp:TemplateField HeaderText="Recipe Type" SortExpression="">
            <ItemTemplate>
                <span runat="server" style="white-space: nowrap;">
                    <asp:HyperLink runat="server" NavigateUrl='<%# "~/RecipeList?type=" + HttpUtility.UrlEncode(Eval("RecipeType.Name").ToString()) %>' Text='<%# Eval("RecipeType.DisplayName") %>'></asp:HyperLink>
                </span>
            </ItemTemplate>
        </asp:TemplateField>
