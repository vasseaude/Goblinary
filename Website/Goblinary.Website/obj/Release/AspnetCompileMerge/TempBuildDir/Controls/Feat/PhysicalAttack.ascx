<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="PhysicalAttack.ascx.cs" Inherits="Goblinary.Website.Controls.Feat.PhysicalAttack" %>
<asp:DetailsView ID="FeatDetailsView" runat="server" AutoGenerateRows="false" CssClass="detailsView">
    ...
    <Fields>
        ...
        <asp:TemplateField HeaderText="Weapon Category:" SortExpression="">
            <ItemTemplate>
                <asp:Label ID="WeaponLabel1" runat="server" Text='<%# Eval("WeaponCategory_Name").ToString() + ((Eval("SpecificWeapon_Name") != null) ? " - <span style=\"color:red;\">" + Eval("SpecificWeapon_Name").ToString() + " Only</span>": "") %>' ></asp:Label>
            </ItemTemplate>
        </asp:TemplateField>
...
<asp:GridView ID="FeatsGridView" runat="server" CssClass="tablesorter custom-popup" AutoGenerateColumns="false">
    <Columns>
        ...
        <asp:BoundField DataField="Role_Name" HeaderText="Role" SortExpression="" />
        <%--keywords--%>
        <asp:TemplateField HeaderText="Weapon" SortExpression="">
            <ItemTemplate>
                <asp:Label ID="WeaponLabel1" runat="server" Text='<%# Eval("WeaponCategory_Name") %>'></asp:Label>
                <asp:Label ID="WeaponLabel2" runat="server" Text='<%# "<br>" + Eval("SpecificWeapon_Name") %>' ForeColor="Red"></asp:Label>
            </ItemTemplate>
        </asp:TemplateField>
