<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="OutpostKit.ascx.cs" Inherits="Goblinary.Website.Controls.Item.Outpost" %>
...
<asp:ListView runat="server" ID="ItemListView">
    ...
    <ItemTemplate>
		<li runat="server" class="detailsListItem">
			<asp:Label runat="server" Text="Structure:" CssClass="detailsListField"></asp:Label>
            <asp:HyperLink runat="server" NavigateUrl='<%# "~/StructureDetails?structure=" + HttpUtility.UrlEncode(Eval("Outpost_Name").ToString()) %>' Text='<%# Eval("Outpost_Name") %>' CssClass="detailsListData" />
		</li>
		<li runat="server" class="detailsListItem">
