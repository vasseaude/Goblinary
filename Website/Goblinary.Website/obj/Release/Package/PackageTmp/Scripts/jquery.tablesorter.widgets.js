...
ts.filter = {
	...
	setDefaults: function(table, c, wo) {
	    ...
	    // begin Kitsune code to override default filters with whatever is in hash
		if (location.hash !== '') {
		    var workingHash = "";
		    var existingHashPieces = [];
		    existingHashPieces = location.hash.split("$");
		    $.each(existingHashPieces, function (index, value) {
		        if (value.indexOf(";" + table.getAttribute("tableName")) > 0) {
		            workingHash += value.replace("#", "").replace(";" + table.getAttribute("tableName"), "");
		        }
		    });
		    var hasharray = workingHash.split("&");
		    $.each(hasharray, function (index, value) {
		        var valueSplit = value.split("=");
		        filters[valueSplit[0].replace("#", "")] = decodeURIComponent(valueSplit[1]);
		    });
		}
	    // end Kitsune code
	...
	checkFilters: function(table, filter, skipFirst) {
	    ...
	    // begin Kitsune code for storing filters in hash
	    if (filters.join('') !== '') {
	        ...
	        if (location.hash !== '') {
	            ...
	            $.each(existingHashPieces, function (index, value) {
	                if (value.indexOf(";" + table.getAttribute("tableName")) > 0) {
	                    workingHash += value.replace("#", "");
	                }
	                else {
	                    everythingElse += value.replace("#", "");
	                    if (value != "") {
	                        everythingElse += "$";
	                    }
	                }
	            ...
	            $.each(filters, function (index, value) {
	                ...
	                if (value != "") {
	                    ...
	                    workingHash += index + "=" + encodeURIComponent(value);
	                    isFirstValue = true;
	                }
	            });
	            workingHash += ";" + table.getAttribute("tableName");
	            workingHash += "$";
	            location.hash = workingHash + everythingElse;
	            workingHash = "";
	        }
	        else {
	            $.each(filters, function (index, value) {
	                ...
	                if (value !== "") {
	                    ...
	                    location.hash += index + "=" + encodeURIComponent(value);
	                    isFirstValue = true;
	                }
	            });
	            location.hash += ";" + table.getAttribute("tableName");
	            location.hash += "$";
	        }
	    }
	    else {
